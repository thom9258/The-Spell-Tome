...
Define TH_ALLOCATOR_IMPLEMENTATION before include.
...
#define TH_INVALID_HANDLE 0

#define TH_BLOCK_ACCESS(allocator_ptr, handle, type)			                   \
	((type*)th_blockallocator_ptr(allocator_ptr, handle))

#define TH_BLOCKALLOCATOR_OPTIMAL_MEMSIZE(n_blocks, block_size)                    \
	( ((n_blocks) * (_TH_ALIGN(block_size))) + ((sizeof(char) * (n_blocks))) )

TH_ALLOCATOR_API
th_status th_blockallocator_setup(th_blockallocator* _ba, uint8_t* _memory, uint32_t _block_size, uint32_t _block_count);

TH_ALLOCATOR_API
void th_blockallocator_destroy(th_blockallocator* _ba);
...
TH_ALLOCATOR_API
th_status
th_blockallocator_setup(th_blockallocator* _ba,
                        uint8_t* _memory,
                        uint32_t _block_size,
                        uint32_t _block_count)
/**
 * th_blockallocator_setup() - Setuo block allocator with provided  memory.
 *
...
TH_ALLOCATOR_API
void
th_blockallocator_destroy(th_blockallocator* _ba)
{
    if (_ba == NULL)
        return;
    *_ba = (th_blockallocator) {0};
}
